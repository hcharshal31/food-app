...
const Cart = () => {
  ...
  return (
    <section className="text-white h-full flex flex-col justify-center">
      ...
      {cartItems.length > 0 && (
        <div className="flex flex-col items-center overflow-y-scroll hide-scrollbar">
          {cartItems.map((item, index) => {
            return (
              <div
                key={item?.id}
                className="w-1/2 h-50 bg-white rounded-xl p-4 mb-5f flex justify-start gap-5 mb-5 relative text-black"
              >
                ...
                <p className="font-semibold absolute top-2 right-2">
                  Count {item?.count}
                </p>
              </div>
            );
          ...
          {cartItems.length > 0 && (
            <div className="flex justify-between w-1/2 h-15 items-center">
              ...
              <button className="bg-red-500 text-white rounded-lg px-4 py-1 mt-2 hover:bg-red-600">
                Pay and Place Order
              </button>
            </div>
          )}
